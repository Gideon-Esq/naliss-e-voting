...
export async function POST(request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  try {
    const result = await db.$transaction(async tx => {
      ...
      return { receipt, candidateName: invite.candidateName, matriculationNumber, position: invite.position.title, level: parsed.data.level, cgpa: parsed.data.cgpa, phone: parsed.data.phone, permanentAddress: parsed.data.permanentAddress, pka: parsed.data.pka, passportData: parsed.data.passportData, tagline: parsed.data.tagline, biography: parsed.data.biography, manifesto: parsed.data.manifesto, mission: parsed.data.mission, vision: parsed.data.vision, priorities: parsed.data.priorities, submittedAt: new Date().toISOString() };

...
export async function POST(request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  if (!parsed.success) return NextResponse.json({ message: "Complete every required field. Manifesto, mission and vision must each contain 400–1000 characters.", issues: parsed.error.flatten().fieldErrors }, { status: 400 });
  if (!image.test(parsed.data.passportData)) return NextResponse.json({ message: "Passport must be a PNG or JPEG image." }, { status: 400 });
  if (!document.test(parsed.data.studentIdData) || !document.test(parsed.data.transcriptData)) return NextResponse.json({ message: "Student ID and academic transcript must be PDF, PNG or JPEG files." }, { status: 400 });
